# ECC v1.5.0 详细参考: 14 Agents + 审查清单 + 生态工具

> **加载时机**: 需要了解 ECC Agent 设计 或 代码审查最佳实践时

## 1. 14 个 Agent 速查 (v1.5.0 Universal Edition)

| Agent | 职责 | 限定工具 |
|-------|------|---------|
| planner | 需求分析 → 实现计划 | Read, Grep, Glob |
| tdd-guide | TDD 红绿重构 | Read, Grep, Bash |
| code-reviewer | 代码审查 | Read, Grep, Glob |
| doc-updater | 变更后更新文档 | Read, Write |
| security-scanner | 安全漏洞检测 | Read, Grep, Bash |
| debugger | 调试定位 | Read, Grep, Bash |
| build-fixer | 构建错误修复 | Read, Write, Bash |
| refactorer | 重构 (单一职责) | Read, Write, Grep |
| git-manager | Git 操作 | Bash |
| ci-helper | CI/CD 管道 | Read, Bash |
| pm2-manager | 进程管理 + Multi-Agent | Bash |
| skill-creator | 从 Git 历史生成 Skill | Read, Write |
| **e2e-tester** | E2E 测试编写和执行 | Read, Write, Bash |
| **architecture-guide** | 架构设计指导 | Read, Grep, Glob |

## 2. Planner Agent 详解

### 职责
- 分析需求 → 创建详细实现计划
- 拆解复杂功能为可管理的步骤
- 识别依赖和潜在风险
- 建议最优实现顺序

### 计划格式 (必须遵循)
```markdown
# Implementation Plan: [Feature Name]

## Overview
[2-3 句摘要]

## Requirements
- [需求 1]

## Architecture Changes
- [变更 1: 文件路径 + 描述]

## Implementation Steps

### Phase 1: [阶段名]
1. **[步骤名]** (File: path/to/file)
   - Action: 具体操作
   - Why: 原因
   - Dependencies: 无 / 依赖步骤 X
   - Risk: Low/Medium/High

## Testing Strategy
- Unit: [要测的文件]
- Integration: [要测的流程]
- E2E: [要测的用户路径]

## Risks & Mitigations
- **Risk**: [描述] → Mitigation: [方案]

## Success Criteria
- [ ] 标准 1
```

### 最佳实践
1. **具体**: 用精确文件路径、函数名、变量名
2. **边界**: 考虑错误场景、null 值、空状态
3. **最小**: 优先扩展现有代码而非重写
4. **模式**: 遵循项目现有约定
5. **可测**: 结构化变更使其易于测试
6. **增量**: 每步可独立验证
7. **记录**: 解释 why 而非仅 what

### 红旗清单 (计划中必须检查)
- 大函数 (>50 行)
- 深嵌套 (>4 层)
- 重复代码
- 缺少错误处理
- 硬编码值
- 缺少测试
- 性能瓶颈
- 无测试策略
- 步骤无明确文件路径
- 阶段无法独立交付

## 3. Code Reviewer Agent 详解

### 审查流程
1. `git diff --staged` + `git diff` 查看所有变更
2. 识别变更文件 → 关联功能 → 理解连接
3. 读周围代码 (不孤立审查, 读完整文件)
4. 按 CRITICAL → LOW 顺序过清单
5. 只报告 **>80% 确信** 是真问题的发现

### 置信度过滤
- **报告**: >80% 确信是真问题
- **跳过**: 风格偏好 (除非违反项目约定)
- **跳过**: 未变更代码中的问题 (除非是安全问题)
- **合并**: 同类问题 (如 "5 个函数缺错误处理" 而非 5 条)
- **优先**: 可能导致 bug / 安全漏洞 / 数据丢失的问题

### 审查清单 — CRITICAL (安全)
- ❌ 硬编码凭据 (API key, 密码, token)
- ❌ SQL 注入 (字符串拼接 vs 参数化查询)
- ❌ XSS (未转义用户输入渲染到 HTML)
- ❌ 路径穿越 (用户控制的文件路径未清理)
- ❌ CSRF (无保护的状态变更端点)
- ❌ 认证绕过 (受保护路由缺 auth 检查)
- ❌ 不安全依赖 (已知漏洞包)
- ❌ 日志泄密 (记录了 token/密码/PII)

### 审查清单 — HIGH (代码质量)
- 大函数 (>50 行) → 拆分
- 大文件 (>800 行) → 提取
- 深嵌套 (>4 层) → 早返回
- 缺少错误处理 → 未处理 Promise
- 变异模式 → 用不可变 (spread/map/filter)
- console.log → 合并前删除
- 缺少测试 → 新路径要有覆盖
- 死代码 → 注释代码/未用 import

### 审查清单 — HIGH (React/Next.js 特定)
- useEffect/useMemo 依赖数组不完整
- render 中调 setState (无限循环)
- 列表用 index 做 key
- Prop 穿透 >3 层 (用 Context)
- 缺少 memo 的昂贵计算
- Server Component 用了 useState/useEffect
- 缺少 Loading/Error/Empty 三态
- 过期闭包 (事件处理器捕获过期状态)

### 审查清单 — HIGH (后端特定)
- 未验证输入 (请求体无 schema 校验)
- 缺少限流 (公开端点无 throttle)
- 无界查询 (SELECT * 无 LIMIT)
- N+1 查询 (循环中查关联数据)
- 缺少超时 (外部 HTTP 调用无 timeout)
- 错误信息泄露 (内部错误发给客户端)
- 缺少 CORS 配置

### 审查清单 — MEDIUM (性能)
- 低效算法 (O(n²) 可优化为 O(n log n))
- 不必要重渲染
- 大 bundle (整包导入而非 tree-shake)
- 缺少缓存
- 未优化图片
- 同步 I/O 在异步上下文中

### 审查输出格式
```markdown
## [FILE_PATH]

### 🔴 CRITICAL: [Issue Title]
**Line**: [number]
**Issue**: [description]
**Fix**: [fix]

---

## Review Summary
| Severity | Count | Status |
|----------|-------|--------|
| CRITICAL | 0     | pass   |
| HIGH     | 2     | warn   |
| MEDIUM   | 3     | info   |
| LOW      | 1     | note   |

Verdict: WARNING — 2 HIGH issues should be resolved before merge.
```

## 4. v1.5.0 生态工具

### AgentShield — 安全审计器
```bash
# 快速扫描 (无需安装)
npx ecc-agentshield scan

# 自动修复安全问题
npx ecc-agentshield scan --fix

# 深度分析 (3 个 Opus Agent 红队/蓝队/审计员)
npx ecc-agentshield scan --opus --stream

# 从零生成安全配置
npx ecc-agentshield init
```

扫描范围: CLAUDE.md, settings.json, MCP configs, hooks, agent definitions, skills
5 类别: 密钥检测 (14 模式), 权限审计, Hook 注入分析, MCP 风险评估, Agent 配置审查
输出: Terminal (A-F 色级), JSON (CI), Markdown, HTML

### Continuous Learning v2 (Instinct 进化系统)
```bash
/instinct-status    # 显示已学习的 instinct + 置信度
/instinct-import    # 从外部导入 instinct
/instinct-export    # 导出 instinct 供分享
/evolve             # 将相关 instinct 聚合为 skill
```

原理: 从 Git 历史自动学习编码模式, 进化为可复用的 Instinct 文件.

### Skill Creator
```bash
/skill-create               # 本地分析当前仓库
/skill-create --instincts   # 同时生成 instinct 文件
```
