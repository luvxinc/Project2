...
**问题:** 多个文件中 `.claude/.claude/projects/` 双层路径导致文件操作失败。

**修复清单:**

| # | 文件 | 行号 | 修改 |
|---|------|------|------|
| 1 | `core/skills/chief-engineer.md` | L203 | `.claude/.claude/projects/` → `.claude/projects/` |
| 2 | `core/skills/project-manager.md` | L243 | `.claude/.claude/projects/` → `.claude/projects/` |
| 3 | `core/skills/project-manager.md` | L276 | `.claude/.claude/projects/` → `.claude/projects/` |
| 4 | `core/skills/project-manager.md` | L308 | `.claude/.claude/projects/` → `.claude/projects/` |
| 5 | `core/skills/qa-auditor.md` | L366 | `.claude/.claude/projects/` → `.claude/projects/` |
| 6 | `core/skills/qa-auditor.md` | L376 | `.claude/.claude/projects/` → `.claude/projects/` |
| 7 | `core/skills/requirements.md` | L155 | `.claude/.claude/projects/` → `.claude/projects/` |
| 8 | `core/skills/memory.md` | 全文扫描 | 统一路径格式 |

**验证:** `grep -r "\.claude/\.claude/" .claude/core/` 应返回零结果。
...
**验证:** `grep -ri "V2\|NestJS\|Prisma" .claude/core/workflows/ .claude/core/skills/` 确认无不当引用。
...
| §2.7 L3 引用表 | −8 | 合并 |
| §2.8 QA 自动化 — 已有 qa-gate.sh | −30 | 只保留 `bash .claude/core/scripts/qa-gate.sh` |
...
**改动文件:** `.claude/projects/mgmt/data/progress/PROJECT-MEMORY.md`
...
1. **记录**: 写入 `.claude/projects/{project}/data/errors/ERROR-BOOK.md`
   - 格式: `memory.md` §3.2
   - 关键词索引更新
...
将 `.claude/core/skills/` 映射为 Antigravity Skills 格式:
...
验收通过:
  .claude/ → .agent.archive/ (归档保留)
  .antigravity/ → 主工作区

验收不通过:
  删除 .antigravity/
  继续使用优化后的 .claude/ (Phase 0-4 的成果仍然有效)
```
...
- [x] 所有 `.claude/.claude/` 路径已修复 (9 处, 跨 5 文件, 含计划外发现的 project-structure.md + requirements.md L57)
- [x] `grep -r "\.claude/\.claude/" .claude/core/` 返回零结果
- [x] 文件行数上限统一标准已写入 (§1 + §9 + qa-auditor 三处统一为 ≤600 行)
- [x] V2 残留引用已清除 (ship.md §1 V2 启动命令已删除)
- [x] `grep -ri "V2.*dev\|NestJS" .claude/core/` 返回零结果 (仅保留 requirements.md 中的"禁止使用 V2"警告规则)
