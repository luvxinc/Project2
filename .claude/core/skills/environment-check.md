...
ls .claude/core/workflows/
ls .claude/core/rules/
ls .claude/projects/{project}/   # {project} 来自 CONTEXT.md §2
```
...
```bash
# 运行文档审计脚本（如果存在）
bash .claude/core/scripts/agent-doc-audit.sh 2>/dev/null || echo "脚本不存在，跳过"
```
...
> **🔴 未扫描 ERROR-BOOK 的任务不得进入 IN_PROGRESS。**

### 扫描流程

1. **提取关键词**: 从 Spec 中提取模块名 + 技术栈 + 操作类型
2. **自动匹配**: `grep -i "{keyword1}\|{keyword2}\|{keyword3}" .claude/projects/{project}/data/errors/ERROR-BOOK.md`
3. **命中处理**: 命中项自动附加到 CTO 工单的"注意事项"
4. **命中格式**: `⚠️ 已知陷阱 ERR-{code}: {标题} — 见 ERROR-BOOK`

```bash
# 自动匹配扫描
keywords=$(echo "{module}|{tech}|{operation}" | tr ' ' '|')
grep -i "$keywords" .claude/projects/{project}/data/errors/ERROR-BOOK.md
```

**通过标准**: 扫描完成（无论是否命中）
**命中已知陷阱**: 附加到 CTO 工单 + 在 Spec "风险与陷阱"中标注
**未扫描**: 🔴 任务不得进入 IN_PROGRESS
...
*Version: 1.1.0 — §3 升级为自动匹配扫描 + 强制门禁*
*Updated: 2026-02-19*
