...
用户在 IN_PROGRESS 期间要求变更时：

| 变更规模 | 判定条件 | PM 行为 |
|---------|---------|--------|
| **小变更** | 不影响架构，不增文件 | 更新 Spec + 通知 CTO |
| **大变更** | 新增模块/改数据模型 | 暂停当前任务 → 走新的 SPEC→CONFIRMED 流程 |

变更记录：在 Spec §5 追加变更日志（时间/变更内容/影响评估/用户确认）。
...
存储 → `.claude/projects/{project}/data/progress/user-feedback.md` (追加)
...
存储 → `.claude/projects/{project}/data/progress/requirements-list.md`
...
存储 → `.claude/projects/{project}/data/progress/risk-register.md`
