---
name: requirements
description: 需求获取与执行向导（5阶段 Wizard）。Use when 需求不完整或存在歧义，需要结构化采集、确认后再执行。
---
...
```
1. 读 `.claude/projects/{project}/CONTEXT.md` §2 “当前阶段指针”
2. 读 `.claude/projects/{project}/CONTEXT.md` §3 “技术栈” → 确认当前运行栈
...
| **相关实施方案** | 检查 `.claude/projects/{project}/playbooks/` | 是否有项目级实施方案 |
| **相关路线图** | 检查 `.claude/projects/{project}/roadmap.md` | 当前阶段是否匹配 |
| **用户历史反馈** | 检查 `data/progress/user-feedback.md` | 用户对类似功能有无偏好/不满 |
| **已知风险** | 检查 `data/progress/risk-register.md` | 当前需求是否涉及已登记风险 |
...
```
.claude/projects/{project}/data/specs/YYYY-MM-DD_{task-name}.md
```
...
> **本 Wizard 对应 `workflows/build.md` 的 DRAFT → SPEC → CONFIRMED 三个状态。**
> **CONFIRMED 后的流程 (分配→执行→验证) 在 build.md §2-§7 定义。**
