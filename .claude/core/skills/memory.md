...
追踪器路径：`.claude/projects/{project}/data/progress/TRACKER-{task-id}.md`
...
TRACKER 增加进度度量（格式见 `tracker-template.md`）：

| 指标 | 说明 |
|------|------|
| Phase 进度 | Phase X of Y |
| 步骤进度 | 当前步骤 / 总步骤 |
| 风险状态 | ✅ 正常 / ⚠️ At Risk / 🔴 Blocked |

"At Risk" 判定：当前步骤耗时 > 预估 2x。
CTO Review SLA：收到完工报告后同一会话内完成审查。
...
2. **验收内容保护** — 将产出文件路径追加到 `.claude/projects/{project}/data/progress/ACCEPTED.md`
3. **删除追踪器** — `rm .claude/projects/{project}/data/progress/TRACKER-{task-id}.md`
4. **删除检查点** — `rm .claude/projects/{project}/data/checkpoints/{相关检查点}.md`
...
```
.claude/projects/{project}/data/errors/ERROR-BOOK.md
```
...
存储：`.claude/projects/{project}/data/progress/PROJECT-MEMORY.md`
