...
> **所有审查/排查任务, 必须以项目架构规范为基准:**
> - 📐 主文件: `{project}/reference/architecture.md` — 见 `CONTEXT.md §7 参考资料索引`
> - 📚 参考规范: `{project}/reference/*.md`
> - 📋 质量基准: `{project}/data/audits/BASELINE-*-deep-quality-audit.md`
> - 🚦 合规门禁: `{project}/reference/architecture-gate.md`
>
> **代码审查/安全审计时, 必须验证代码符合项目架构门禁规则（`architecture-gate.md`）。不合规 = Block。**
...
> **独立入口:** `/review` — 可直接调用，自带完整审查清单 (S1-S8 / Q1-Q8 / R1-R7 / K1-K7 / P1-P6)
...
| 级别 | 检查内容 | 规则真相源 |
|------|---------|-----------|
| 🔴 CRITICAL | 安全漏洞 (注入/XSS/CSRF/凭据泄露/认证绕过/日志泄密) | `rules/common.md` §4 |
| 🟠 HIGH | 代码质量 (大函数>50行/大文件>600行/深嵌套>4层/缺错误处理/死代码) | `rules/common.md` §1 |
| 🟠 HIGH | React/Next.js (依赖数组/三态/Server Component/过期闭包) | `rules/frontend.md` §1-§2 |
| 🟠 HIGH | 后端 (N+1/无界查询/事务/输入验证/同步I/O阻塞/缓存) | `rules/backend.md` §1-§2 |
| 🟡 MEDIUM | 性能 (低效算法/不必要重渲染/大bundle/缺缓存/未优化图片) | `rules/frontend.md` §3, `rules/backend.md` §3 |
| 🟢 LOW | 风格 (命名/注释/格式) — 记录但不阻塞 | — |

> **完整审查清单 (含每项具体检测方式):** `/review` skill 内联清单
...
### L3 参考
- ECC Code Reviewer 详解: `warehouse/tools/everything-claude-code/01-agents-review.md` §3
...
4. 修复 → 验证 (6 阶段验证循环)
5. 确认无回归
6. 🔴 问题复盘铁律 → `memory.md §3.2` 记录 + `memory.md §3.5` 交叉检查
...
> **🔴 出错时必读:** `core/reference/root-cause-classifier.md` — 先分类根因，再决定行动。禁止未诊断直接改代码。

### 排查流程

```
0. 🔴 根因分类 (FIRST): 加载 root-cause-classifier.md → 判断类型 A/B/C/D/E/F
   → 🧠 THINK (rules/common.md §12): 根因确认了吗? 修的是症状还是根因? 有同类代码吗?
   → 类型确认后，按对应 SOP 诊断，再执行以下步骤
...
关键规则: 不猜，用日志和数据说话；先理解，再修复；修复后补测试。
...
*Version: 2.4.0 — §2 审查增强 + §5 Think 检查点 (Anthropic Agent Bible L2 对齐)*
