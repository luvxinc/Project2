...
case "$MODE" in
  init)
    ...
    SCOPE_INIT="/Users/aaron/Developer/MGMTV2/.claude/core/scripts/scope-contract-init.sh"
    if [ -x "$SCOPE_INIT" ]; then
      "$SCOPE_INIT" "$PROOT" "$TASK_ID" "$INIT_BASE_REF" \
        '^\.claude/core/scripts/.*' \
        '^\.claude/core/reference/.*' \
        '^\.claude/projects/.*/data/(plans|audits|tmp)/.*' >/dev/null || true
