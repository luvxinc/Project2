...
def upload_po_invoice_api(request):
    """
    上传发货单账单文件
    URL: /dashboard/purchase/api/send_mgmt/upload_invoice/
    POST multipart/form-data: { logistic_num, invoice_file }
    
    保存路径: data/logistic/{logistic_num}_invoice_Ver01.{ext}
    """
    ...
    is_valid, msg = SecurityPolicyManager.verify_action_request(request, 'btn_send_upload_invoice')
    if not is_valid:
    ...
    try:
        ...
        # 确定年份（从 date_sent 或当前年份）
        from datetime import datetime
        year = str(datetime.now().year)
        
        # 新路径：data/records/purchase/send/{YYYY}/
        invoices_dir = settings.DATA_DIR / 'records' / 'purchase' / 'send' / year
        invoices_dir.mkdir(parents=True, exist_ok=True)
        
        # 查找现有版本（兼容新旧格式）
        version = 1
        existing_files = list(invoices_dir.glob(f"{logistic_num}_invoice_V*"))
        if existing_files:
        ...
        # 生成文件名（新格式：_V##）
        filename = f"{logistic_num}_invoice_V{str(version).zfill(2)}{ext}"
...
def get_invoice_info_api(request):
    """
    获取发货单账单文件信息
    URL: /dashboard/purchase/api/send_mgmt/invoice_info/?logistic_num=xxx
    """
    if not check_perm(request.user, 'module.purchase.send.mgmt'):
        return JsonResponse({'success': False, 'message': _('无权限')}, status=403)
...
def serve_invoice_file_api(request):
    """
    提供账单文件下载/查看
    URL: /dashboard/purchase/api/send_mgmt/serve_invoice/?logistic_num=xxx&filename=xxx
    """
    if not check_perm(request.user, 'module.purchase.send.mgmt'):
        return JsonResponse({'success': False, 'message': _('无权限')}, status=403)
    ...
    if not logistic_num or not filename:
    ...
    try:
        ...
        # 新路径：data/records/purchase/send/{YYYY}/filename
        filepath = settings.DATA_DIR / 'records' / 'purchase' / 'send' / year / filename
        
        # 安全检查：确保文件名以logistic_num开头
        if not filename.startswith(logistic_num):
...
@login_required(login_url='web_ui:login')
@require_http_methods(["POST"])
def delete_send_invoice_api(request):
    """
    删除发货单账单文件
    URL: /dashboard/purchase/api/send_mgmt/delete_invoice/
    POST JSON: { logistic_num, filename }
    """
    import json
    ...
    try:
        ...
        # [Security Fix 2026-01-11] 添加密码验证
        is_valid, msg = SecurityPolicyManager.verify_action_request(request, 'btn_send_delete_invoice', data)
        if not is_valid:
            return JsonResponse({'success': False, 'message': msg or _('密码验证失败')}, status=403)
