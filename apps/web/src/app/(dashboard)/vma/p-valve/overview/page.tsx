...
export default function InventoryOverviewPage() {
  ...
  const ACTION_LABEL: Record<string, string> = useMemo(() => ({
    REC_CN: t('p_valve.overview.actions.recCn'),
    REC_CASE: t('p_valve.overview.actions.recCase'),
    OUT_CASE: t('p_valve.overview.actions.outCase'),
    OUT_CN: t('p_valve.overview.actions.outCn'),
    USED_CASE: t('p_valve.overview.actions.usedCase'),
    MOVE_DEMO: t('p_valve.overview.actions.moveDemo'),
  }), [t]);
    ...
    <div style={{ backgroundColor: colors.bg }} className="min-h-screen pb-20">
      ...
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <h1 className="text-[18px] font-semibold" style={{ color: colors.text }}>{t('p_valve.overview.title')}</h1>
          <button onClick={refetch}
            className="px-3 py-1.5 rounded-lg text-[12px] font-medium border hover:opacity-80 transition"
            style={{ ...inputSx }}>
            {t('p_valve.overview.refresh')}
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <span className="ml-auto text-[11px] tabular-nums" style={{ color: colors.textTertiary }}>
            {t('p_valve.overview.filters.recordCount', { count: filtered.length })}
          </span>
        </div>
        ...
        <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: colors.bgSecondary, borderColor: colors.border }}>
          {loading ? (
            <div className="flex items-center justify-center py-20">
              <span className="text-[13px]" style={{ color: colors.textTertiary }}>{t('p_valve.overview.loading')}</span>
            </div>
          ) : rows.length === 0 ? (
            <div className="flex flex-col items-center py-20" style={{ color: colors.textTertiary }}>
              <p className="text-[14px] font-medium">{hasFilter ? t('p_valve.overview.noMatch') : t('p_valve.overview.empty')}</p>
            </div>
          ) : (
            <table className="w-full text-[12px]" style={{ color: colors.text }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${colors.border}` }}>
                  {[t('p_valve.overview.columns.date'), t('p_valve.overview.columns.action'), t('p_valve.overview.columns.type'), t('p_valve.overview.columns.specNo'), t('p_valve.overview.columns.serialNo'), t('p_valve.overview.columns.qty'), t('p_valve.overview.columns.batchNo'), t('p_valve.overview.columns.expDate'), t('p_valve.overview.columns.operator'), t('p_valve.overview.columns.notes')].map(h => (
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                ...
                style={inputSx}>{t('p_valve.overview.pagination.prev')}</button>
              <span className="text-[11px] tabular-nums" style={{ color: colors.textSecondary }}>{page}/{totalPages}</span>
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                className="px-2.5 py-1 rounded-lg text-[11px] border disabled:opacity-30 hover:opacity-80 transition"
                style={inputSx}>{t('p_valve.overview.pagination.next')}</button>
            </div>
          </div>
        )}
      </div>
