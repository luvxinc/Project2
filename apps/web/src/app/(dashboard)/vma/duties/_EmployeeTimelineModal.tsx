...
interface EmployeeDetail {
  ...
  departmentAssignments: {
    id: string;
    department: { id: string; code: string; name: string; duties: string };
    assignedAt: string;
...
export default function EmployeeTimelineModal({
  ...
  // Editor - Edit Mode (Update past events)
  const [editGroup, setEditGroup] = useState<HistoryGroup | null>(null);
  const [editingEventId, setEditingEventId] = useState<string | null>(null); // Which item is being edited
  ...
  const fetchData = useCallback(async () => {
    try {
      const [empData, deptData] = await Promise.all([
        api<EmployeeDetail>(`/vma/employees/${employeeId}`),
        api<Department[]>('/vma/departments')
      ]);
      setEmployee(empData);
  ...
  const openEditGroup = (group: HistoryGroup) => {
      ...
      setEditingEventId(null); // Reset inline edit
      setEditorOpen(true);
  };
  ...
  const startEditEvent = (evt: HistoryGroup['events'][0]) => {
      if (evt.type === 'JOINED' && evt.assignmentId) {
          ...
          alert(t('employees.career.alertOnlyAssigned') || "Only 'Assigned Duty' events can be modified to a different function.");
      }
  };
  ...
  const handleCreate = async () => {
      ...
      try {
          if (actionType === 'ASSIGN') {
              ...
              await api(`/vma/employees/${employeeId}/departments`, { method: 'POST', body: JSON.stringify({ departmentId: targetDeptId, assignedAt: eventDate }) });
              ...
              await api(`/vma/employees/${employeeId}/toggle`, { method: 'PATCH', body: JSON.stringify({ status: newStatus, date: eventDate }) });
          }
          setEditorOpen(false);
          setAnimated(false);
          fetchData();
  ...
  const handleUpdateEvent = async () => {
      ...
      setSaving(true);
      try {
          // Send PATCH to update department
          // Note: Backend might need to support this. If not, we error.
          await api(`/vma/employee-departments/${editingEventId}`, { 
              method: 'PATCH', 
              body: JSON.stringify({ departmentId: updateDeptId }) 
          });
          setEditorOpen(false);
          setAnimated(false);
          fetchData();
  ...
  const colors = themeColors[theme as 'dark' | 'light'];
                    ...
                    {!editGroup ? (
                        ...
                        <>
                            <div className="grid grid-cols-3 gap-3 mb-6">
                                <button onClick={() => setActionType('ASSIGN')} 
                                        className={`py-3 rounded-xl text-sm font-bold border transition-all ${actionType === 'ASSIGN' ? 'border-blue-500 shadow-lg' : 'bg-gray-100 dark:bg-white/5 border-transparent opacity-60'}`}
                                        style={actionType === 'ASSIGN' ? { backgroundColor: colors.blue, color: colors.white } : undefined}>
                                    {t('employees.career.addDuty') || 'Add Duty +'}
                                </button>
                                <button onClick={() => setActionType('REMOVE')} disabled={!canRemoveDuty}
                                        className={`py-3 rounded-xl text-sm font-bold border transition-all ${!canRemoveDuty ? 'opacity-30 cursor-not-allowed bg-gray-100' : actionType === 'REMOVE' ? 'border-orange-500 shadow-lg' : 'bg-gray-100 dark:bg-white/5 border-transparent opacity-60'}`}
                                        style={actionType === 'REMOVE' && canRemoveDuty ? { backgroundColor: colors.orange, color: colors.white } : undefined}>
                                    {t('employees.career.removeDuty') || 'Remove Duty -'}
                                </button>
                                <button onClick={() => setActionType('STATUS')} 
                                        className={`py-3 rounded-xl text-sm font-bold border transition-all ${actionType === 'STATUS' ? 'border-purple-500 shadow-lg' : 'bg-gray-100 dark:bg-white/5 border-transparent opacity-60'}`}
                                        style={actionType === 'STATUS' ? { backgroundColor: colors.indigo, color: colors.white } : undefined}>
                                    {t('employees.career.terminateStatus') || 'Terminate / Status'}
                                </button>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input type="date" value={eventDate} onChange={e => setEventDate(e.target.value)} 
                                           className="w-full px-4 py-3 rounded-xl bg-gray-100 dark:bg-white/5 border-none outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                                ...
                                {actionType === 'ASSIGN' && (
                                    <div>
                                        ...
                                        <select value={targetDeptId} onChange={e => setTargetDeptId(e.target.value)} 
                                            ...
                                            {allDepts.map(d => (<option key={d.id} value={d.id}>{d.code} - {d.duties}</option>))}
                                        </select>
                                    </div>
                                )}
                                ...
                                {actionType === 'REMOVE' && canRemoveDuty && (
                                    <div>
                                        ...
                                        <select value={targetAssignmentId} onChange={e => setTargetAssignmentId(e.target.value)} 
                                                className="w-full px-4 py-3 rounded-xl bg-gray-100 dark:bg-white/5 border-none outline-none focus:ring-2 focus:ring-orange-500">
                                            <option value="">{t('employees.career.selectPlaceholder') || 'Select...'}</option>
                                ...
                                {actionType === 'STATUS' && (
                                     <div>
                                        ...
                                        <select value={newStatus} onChange={e => setNewStatus(e.target.value)} 
                                                className="w-full px-4 py-3 rounded-xl bg-gray-100 dark:bg-white/5 border-none outline-none focus:ring-2 focus:ring-purple-500">
                            ...
                            <div className="mt-8">
                                <button onClick={handleCreate} disabled={saving || (!canRemoveDuty && actionType === 'REMOVE')} 
                                        className="w-full py-4 rounded-xl font-bold text-lg shadow-xl transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                                        style={{ backgroundColor: actionType === 'ASSIGN' ? colors.blue : actionType === 'REMOVE' ? colors.orange : colors.indigo, color: colors.white }}>
                                    {saving ? (t('employees.career.processing') || 'Processing...') : (t('employees.career.confirmPlan') || 'Confirm Plan')}
                                </button>
                            </div>
                        </>
                    ) : (
                        // EDIT MODE (Past History)
                        <div className="space-y-4">
                                     ...
                                     <div className="flex justify-between items-start mb-2">
                                         ...
                                         {evt.type === 'JOINED' && evt.assignmentId && editingEventId !== evt.assignmentId && (
                                             <button onClick={() => startEditEvent(evt)}
                                                 ...
                                                 {t('employees.career.changeDuty') || 'Change Duty'}
                                             </button>
                                         )}
                                     </div>
                                     
                                     {/* Inline Editor for Changing Dept */}
                                     {editingEventId === evt.assignmentId && (
                                         <div className="mt-3 pt-3 border-t border-dashed border-gray-500/20 animate-in slide-in-from-top-2">
                                              ...
                                              <div className="flex gap-2">
                                                  <select value={updateDeptId} onChange={e => setUpdateDeptId(e.target.value)}
                                                          className="flex-1 px-3 py-2 rounded-lg bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 text-sm">
                                                      {allDepts.map(d => (<option key={d.id} value={d.id}>{d.code} - {d.duties}</option>))}
                                                  </select>
                                                  <button onClick={handleUpdateEvent} disabled={saving}
                                                          className="px-4 py-2 text-sm font-bold rounded-lg hover:opacity-90"
                                                          style={{ backgroundColor: colors.blue, color: colors.white }}>
                                                      {t('employees.career.save') || 'Save'}
                                                  </button>
                                                  <button onClick={() => setEditingEventId(null)}
                                                          className="px-4 py-2 bg-gray-200 dark:bg-white/10 text-sm font-bold rounded-lg hover:opacity-70">
                                                      {t('employees.career.cancel') || 'Cancel'}
                                                  </button>
                                              </div>
                                              <div className="mt-2 text-[10px] text-orange-500 font-medium">
                                                  {t('employees.career.historyAdjustment') || '* History adjustment: This will update the official record for this date.'}
                                              </div>
                                         </div>
                                     )}
                                 </div>
                             ))}
                             <div className="mt-6 text-xs text-center opacity-40 px-8">
                                 {t('employees.career.cannotDeleteHistory') || 'Historical records cannot be deleted. You may correct the duty assignment.'}
                             </div>
                        </div>
                    )}
