                        ...
                        <>
                            <div className="grid grid-cols-3 gap-3 mb-6">
                                <button onClick={() => setActionType('ASSIGN')} 
                                        className={`py-3 rounded-xl text-sm font-bold border transition-all ${actionType === 'ASSIGN' ? 'bg-blue-500 text-white border-blue-500 shadow-lg' : 'bg-gray-100 dark:bg-white/5 border-transparent opacity-60'}`}>
                                    {t('employees.career.addDuty') || 'Add Duty +'}
                                </button>
                                <button onClick={() => setActionType('REMOVE')} disabled={!canRemoveDuty}
                                        className={`py-3 rounded-xl text-sm font-bold border transition-all ${!canRemoveDuty ? 'opacity-30 cursor-not-allowed bg-gray-100' : actionType === 'REMOVE' ? 'bg-orange-500 text-white border-orange-500 shadow-lg' : 'bg-gray-100 dark:bg-white/5 border-transparent opacity-60'}`}>
                                    {t('employees.career.removeDuty') || 'Remove Duty -'}
                                </button>
                                <button onClick={() => setActionType('STATUS')} 
                                        className={`py-3 rounded-xl text-sm font-bold border transition-all ${actionType === 'STATUS' ? 'bg-purple-500 text-white border-purple-500 shadow-lg' : 'bg-gray-100 dark:bg-white/5 border-transparent opacity-60'}`}>
                            ...
                            <div className="mt-8">
                                <button onClick={handleCreate} disabled={saving || (!canRemoveDuty && actionType === 'REMOVE')} 
                                        className="w-full py-4 rounded-xl font-bold text-lg text-white shadow-xl transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                                        style={{ backgroundColor: actionType === 'ASSIGN' ? colors.blue : actionType === 'REMOVE' ? colors.orange : colors.indigo }}>
                                     ...
                                     <div className="flex justify-between items-start mb-2">
                                         ...
                                         {evt.type === 'JOINED' && evt.assignmentId && editingEventId !== evt.assignmentId && (
                                             <button onClick={() => startEditEvent(evt)}
                                                     className="px-3 py-1.5 bg-blue-500/10 text-blue-500 text-xs font-bold rounded-lg hover:bg-blue-500 hover:text-white transition">
