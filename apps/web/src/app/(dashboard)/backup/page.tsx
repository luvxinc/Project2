...
export default function BackupPage() {
  ...
  useEffect(() => {
    if (!loading && backups.length > 0 && !isFlipped) {
  ...
  }, [loading, backups, isFlipped]);

  // ── Slide-in detail (V3 standard pattern) ──────────────

  const openDetail = async (backup: BackupListItem) => {
    setDetailLoading(true);
    setSelectedBackup(null);

    // Slide front out to the left
    if (frontRef.current) {
  ...
  return (
    <div style={{ backgroundColor: colors.bg }} className="min-h-screen pb-20 overflow-x-hidden">
      {/* ── Header (only when list is visible) ──────────── */}
      {!isFlipped && (
      ...
      {/* ── Content Area ──────────────────────────────────── */}
      <section className="max-w-[1000px] mx-auto px-6 py-6 relative">
        ...
        <div className="relative z-20">
          ...
          {/* BACK: Detail Panel (V3 standard slide-in) */}
          {isFlipped && (
            <div ref={backRef}>
