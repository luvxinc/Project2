...
import type { POPaymentListItem, FileItem } from '@/lib/api';
import HistoryPanel from './HistoryPanel';
import OrdersPanel from './OrdersPanel';

interface POPaymentDetailPanelProps {
  pmtNo: string;
  poNum: string;
...
export default function POPaymentDetailPanel({
  pmtNo, poNum, item, onBack, onDeletePayment, t, theme,
}: POPaymentDetailPanelProps) {
  ...
  const { data: filesData, isLoading: filesLoading } = useQuery({
    queryKey: ['poPaymentFiles', pmtNo],
    queryFn: () => financeApi.getPOPaymentFiles(pmtNo),
  ...
  const uploadSecurity = useSecurityAction({
    actionKey: 'btn_po_payment_upload_file',
    level: 'L2',
    onExecute: async (code) => {
      if (!pendingFileRef.current) return;
      setUploadPending(true);
      try {
        await financeApi.uploadPOPaymentFile(pmtNo, pendingFileRef.current, code);
        queryClient.invalidateQueries({ queryKey: ['poPaymentFiles', pmtNo] });
      } finally {
  ...
  const deleteMutation = useMutation({
    mutationFn: (args: { filename: string; code: string }) =>
      financeApi.deletePOPaymentFile(pmtNo, args.filename, args.code),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['poPaymentFiles', pmtNo] });
  ...
  const deleteFileSecurity = useSecurityAction({
    actionKey: 'btn_po_payment_delete_file',
    level: 'L2',
    onExecute: (code) => {
  ...
  const handleDownload = (filename: string) => {
    const url = financeApi.servePOPaymentFile(pmtNo, filename);
    ...
    <div className="relative">
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            >
              ...
              />
              {t(`poPayment.status.${item.paymentStatus}`)}
            </span>
          </div>
          <div className="text-right">
            <p className="text-xs" style={{ color: colors.textTertiary }}>{t('poPayment.detail.totalAmount')}</p>
            <p className="text-sm font-mono font-semibold tabular-nums" style={{ color: colors.text }}>
              ${fmtNum(item.totalAmountUsd, 5)}
              <span className="ml-2 text-xs font-normal" style={{ color: colors.textTertiary }}>
                ≈ ¥{fmtNum(item.totalAmountRmb, 5)}
              </span>
            </p>
          </div>
        </div>
        ...
        <div className="p-5 grid grid-cols-2 md:grid-cols-4 gap-4">
          <FieldBlock label={t('poPayment.detail.supplier')} value={`${item.supplierCode} — ${item.supplierName}`} colors={colors} />
          <FieldBlock label={t('poPayment.detail.poDate')} value={item.poDate} colors={colors} />
          <FieldBlock label={t('poPayment.detail.currency')} value={item.curCurrency} colors={colors} />
          <FieldBlock
            label={t('poPayment.detail.exchangeRate')}
            value={fmtNum(item.curUsdRmb, 4)}
            suffix={item.rateSourceCode === 'A' || item.rateSourceCode === 'auto'
              ? t('poPayment.detail.rateAuto')
              : t('poPayment.detail.rateManual')}
            colors={colors}
            mono
          />
          <DualCurrencyBlock label={t('poPayment.detail.depositPaid')} usd={item.depositPaidUsd} rmb={item.depositPaid} colors={colors} />
          <DualCurrencyBlock label={t('poPayment.detail.poPaid')} usd={item.poPaidUsd} rmb={item.poPaid} colors={colors} />
          <DualCurrencyBlock label={t('poPayment.detail.balanceRemaining')} usd={item.balanceRemainingUsd} rmb={item.balanceRemaining} colors={colors} />
          <DualCurrencyBlock
            label={t('poPayment.detail.extraFees')}
            usd={item.extraFeesUsd}
            rmb={item.extraFeesRmb}
            dash={item.extraFeesUsd <= 0}
            colors={colors}
          />
          <FieldBlock label={t('poPayment.detail.paymentDate')} value={item.latestPaymentDate || '—'} colors={colors} />
        </div>
      </div>

      {/* ── Deposit Payments Section ── */}
      <div
        className="rounded-xl mb-5"
        style={{ backgroundColor: colors.bgSecondary, border: `1px solid ${colors.border}` }}
      >
        <div className="px-5 py-3" style={{ borderBottom: `1px solid ${colors.border}` }}>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold" style={{ color: colors.text }}>
              {t('poPayment.detail.depositDetailsTitle')}
            </h3>
            {item.depositDetails && item.depositDetails.length > 0 && (
              <span
                className="text-[10px] px-1.5 py-0.5 rounded-full font-medium"
                style={{ backgroundColor: `${colors.green}15`, color: colors.green }}
              >
                {item.depositDetails.length}
              </span>
            )}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ backgroundColor: `${colors.bg}50` }}>
                  <th className="text-left py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.pmtNo')}</th>
                  <th className="text-left py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.depPmtDate')}</th>
                  <th className="text-left py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.depPmtCur')}</th>
                  <th className="text-right py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.depPmtPaid')}</th>
                  <th className="text-right py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.depPmtPaidCur')}</th>
                  <th className="text-right py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.depPmtPrepayAmount')}</th>
                  <th className="text-center py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.depPmtOverride')}</th>
                  <th className="text-right py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.extraAmount')}</th>
                </tr>
              </thead>
              <tbody>
                {item.depositDetails.map((det, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 px-4 text-center whitespace-nowrap">
                        {det.depOverride === 1 ? (
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded" style={{ backgroundColor: hexToRgba(colors.red, 0.12), color: colors.red }}>{t('poPayment.detail.depPmtOverride')}</span>
                        ) : (
      ...
      >
        <div className="px-5 py-3" style={{ borderBottom: `1px solid ${colors.border}` }}>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold" style={{ color: colors.text }}>
              {t('poPayment.detail.paymentDetailsTitle')}
            </h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr style={{ backgroundColor: `${colors.bg}50` }}>
                  <th className="text-left py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.pmtNo')}</th>
                  <th className="text-left py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.poPmtDate')}</th>
                  <th className="text-left py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.poPmtCur')}</th>
                  <th className="text-right py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.poPmtPaid')}</th>
                  <th className="text-right py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.poPmtPaidCur')}</th>
                  <th className="text-right py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.poPmtPrepayAmount')}</th>
                  <th className="text-center py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.poPmtOverride')}</th>
                  <th className="text-right py-2.5 px-4 text-xs font-medium uppercase tracking-wider" style={{ color: colors.textTertiary }}>{t('poPayment.detail.extraAmount')}</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    >
                      ...
                      <td className="py-2 px-4 whitespace-nowrap">
                        <span
                          className="text-[10px] font-medium px-1.5 py-0.5 rounded"
                          style={{
                            backgroundColor: currencyBadgeStyle(det.poCur, colors).bg,
                            color: currencyBadgeStyle(det.poCur, colors).color,
                          }}
                        >
                          {curSym(det.poCur)}
                        </span>
                      ...
                      <td className="py-2 px-4 text-right whitespace-nowrap">
                        <span style={{ color: colors.text }} className="font-mono text-xs tabular-nums">
                          {curSym(det.poCur)}{fmtNum(det.poPaid)}
                        </span>
                      </td>
                      <td style={{ color: colors.textSecondary }} className="py-2 px-4 text-xs font-mono text-right whitespace-nowrap tabular-nums">
                        {curSym(det.poCur)}{fmtNum(det.poPaidCur, 4)}
                      </td>
                      <td className="py-2 px-4 text-right whitespace-nowrap">
                        <span
                          ...
                          className="font-mono text-xs tabular-nums"
                        >
                          {det.poPrepayAmount > 0 ? `$${fmtNum(det.poPrepayAmount)}` : '—'}
                        </span>
                      </td>
                      <td className="py-2 px-4 text-center whitespace-nowrap">
                        {det.poOverride === 1 ? (
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded" style={{ backgroundColor: hexToRgba(colors.red, 0.12), color: colors.red }}>{t('poPayment.detail.poPmtOverride')}</span>
                        ) : (
                          <span style={{ color: colors.textTertiary }} className="text-xs">—</span>
                        )}
                      </td>
      ...
      {(() => {
        const tabs = [
          { id: 'history' as const, label: t('poPayment.detail.tab_history') },
          { id: 'orders' as const, label: t('poPayment.detail.tab_orders') },
          { id: 'files' as const, label: t('poPayment.detail.tab_files'), count: fileCount },
        ];
        return (
      ...
      <SecurityCodeDialog
        ...
        title={t('poPayment.files.uploadTitle')}
        description={t('poPayment.files.uploadDesc')}
      ...
      <SecurityCodeDialog
        ...
        title={t('poPayment.files.deleteTitle')}
        description={t('poPayment.files.deleteDesc')}
...
function DualCurrencyBlock({ label, usd, rmb, dash, colors }: {
  label: string; usd: number; rmb: number; dash?: boolean;
  colors: (typeof themeColors)['dark'];
}) {
  return (
    <div>
      <p className="text-xs font-medium mb-1" style={{ color: colors.textTertiary }}>{label}</p>
      <p className="text-sm font-mono tabular-nums" style={{ color: colors.text }}>
        {dash ? '—' : (
          <>
            ${fmtNum(usd, 5)}
            <span className="ml-2 text-xs font-normal" style={{ color: colors.textTertiary }}>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs" style={{ color: colors.textTertiary }}>
          {t('poPayment.files.count', { count: files.length })}
        </p>
        <button
          onClick={onUpload}
        ...
        >
          {uploadPending ? (
            <div className="w-3 h-3 border-2 rounded-full animate-spin" style={{ borderColor: 'transparent', borderTopColor: colors.blue }} />
          ) : (
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
          )}
          {t('poPayment.files.upload')}
        </button>
      </div>

      {files.length === 0 ? (
        <p className="py-10 text-center text-sm" style={{ color: colors.textTertiary }}>
          {t('poPayment.files.noFiles')}
        </p>
      ) : (
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0 ml-4">
                <button onClick={() => onDownload(file.name)}
                  ...
                  style={{ backgroundColor: colors.bgTertiary }} title={t('poPayment.files.download')}>
                ...
                <button onClick={() => onDelete(file.name)}
                  className="p-1.5 rounded-lg transition-opacity hover:opacity-70"
                  style={{ backgroundColor: colors.bgTertiary }} title={t('poPayment.files.delete')}>
