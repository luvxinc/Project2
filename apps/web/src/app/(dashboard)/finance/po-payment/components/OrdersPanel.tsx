...
/**
 * OrdersPanel -- Related PO details for a PO payment batch.
 * V1 parity: po_payment/history.py po_payment_orders_api
 *
...
export default function OrdersPanel({ pmtNo, t, theme }: OrdersPanelProps) {
  ...
  const { data: ordersData, isLoading } = useQuery({
    queryKey: ['poPaymentOrders', pmtNo],
    queryFn: () => financeApi.getPOPaymentOrders(pmtNo),
  });
    ...
    <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${colors.border}` }}>
      ...
      >
        <div className="flex items-center gap-4">
          <div>
            <span className="text-xs" style={{ color: colors.textTertiary }}>{t('poPayment.orders.poNum')}</span>
            <p className="text-sm font-mono font-medium" style={{ color: colors.blue }}>{order.poNum}</p>
          </div>
          <div>
            <span className="text-xs" style={{ color: colors.textTertiary }}>{t('poPayment.orders.supplier')}</span>
            <p className="text-sm font-mono" style={{ color: colors.text }}>{order.supplierCode}</p>
          </div>
          <div>
            <span className="text-xs" style={{ color: colors.textTertiary }}>{t('poPayment.orders.orderDate')}</span>
            <p className="text-sm" style={{ color: colors.text }}>{order.poDate}</p>
          </div>
          <div>
            <span className="text-xs" style={{ color: colors.textTertiary }}>{t('poPayment.orders.currency')}</span>
            <p className="text-sm" style={{ color: colors.text }}>{order.currency}</p>
          </div>
          <div>
            <span className="text-xs" style={{ color: colors.textTertiary }}>{t('poPayment.orders.rate')}</span>
            <p className="text-sm tabular-nums" style={{ color: colors.text }}>{order.exchangeRate.toFixed(4)}</p>
          </div>
          <div>
            <span className="text-xs" style={{ color: colors.textTertiary }}>{t('poPayment.orders.depositPercent')}</span>
        ...
        <div className="text-right">
          <p className="text-xs tabular-nums" style={{ color: colors.textSecondary }}>
            {t('poPayment.orders.totalRmb')}: <span className="font-medium" style={{ color: colors.text }}>¥{fmtNum(order.totalRmb, 5)}</span>
          </p>
          <p className="text-xs tabular-nums" style={{ color: colors.textSecondary }}>
            {t('poPayment.orders.totalUsd')}: <span className="font-medium" style={{ color: colors.text }}>${fmtNum(order.totalUsd, 5)}</span>
          </p>
        </div>
      </div>
