...
/**
 * Group paid deposit items by their payment number (pmtNo from paymentDetails).
 */
export interface DepositPaymentGroup {
  pmtNo: string;
  paymentDate: string;
  supplierCode: string;
  supplierName: string;
  items: DepositListItem[];
  totalDepositRmb: number;
  totalDepositUsd: number;
  totalActualPaidUsd: number;
  totalPrepayDeductedUsd: number;
  totalExtraFeesUsd: number;
  exchangeRate: number;
}

function groupPaidByPayment(items: DepositListItem[]): DepositPaymentGroup[] {
  const map = new Map<string, DepositListItem[]>();

  for (const item of items) {
    if (item.paymentDetails && item.paymentDetails.length > 0) {
      // Group by the first payment pmtNo (a PO may have multiple payment records)
  ...
  const groups: DepositPaymentGroup[] = [];
  for (const [pmtNo, groupItems] of map) {
    // Deduplicate items (same PO may appear via multiple payment details)
    const seen = new Set<string>();
    const uniqueItems: DepositListItem[] = [];
    ...
    let totalDepositRmb = 0;
    let totalDepositUsd = 0;
    let totalActualPaidUsd = 0;
    let totalPrepayDeductedUsd = 0;
    ...
    for (const item of uniqueItems) {
      totalDepositRmb += item.depositAmountRmb;
      totalDepositUsd += item.depositAmountUsd;
      totalActualPaidUsd += item.actualPaidUsd;
      totalPrepayDeductedUsd += item.prepayDeductedUsd;
 ...
 * V1 parity: backend/templates/finance/pages/deposit.html
 *
 * Design: Apple-style with 2-level slide-in navigation.
 *   Level 1: Unpaid supplier-grouped cards (selectable) + Paid payment table
 *   Level 2: DepositDetailPanel (tabs: History | Orders | Files)
 */
export default function DepositPage() {
  ...
  // Client-side search filtering + split into unpaid / paid
  const { unpaidData, paidData } = useMemo(() => {
    const list = depositData?.data || [];
    ...
    const unpaid: DepositListItem[] = [];
    const paid: DepositListItem[] = [];
  ...
  // Group paid items by payment for table display
  const paymentGroups = useMemo(() => groupPaidByPayment(paidData), [paidData]);

  const totalCount = depositData?.data?.length ?? 0;
  ...
  // List → Payment Detail (paid payment group click)
  const handlePaymentRowClick = useCallback((group: DepositPaymentGroup) => {
    slideForward(() => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (secCode: string) =>
      financeApi.deleteDepositPayment(deleteTarget!, secCode),
    onSuccess: () => {
  ...
  const deleteSecurity = useSecurityAction({
    actionKey: 'btn_deposit_payment_delete',
    level: 'L3',
  ...
  return (
    <div style={{ backgroundColor: colors.bg }} className="min-h-screen pb-20 overflow-x-hidden">
      ...
      {viewMode === 'list' && (
        <section className="max-w-[1400px] mx-auto px-6 pb-4">
          <span style={{ color: colors.textTertiary }} className="text-sm">
            {t('deposit.table.count', { count: totalCount })}
          </span>
        </section>
      )}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative flex-1 min-w-[200px] max-w-[320px]">
              <input
                ...
                placeholder={t('deposit.table.search')}
            ...
            >
              ...
              {t('deposit.submitPayment')}
            </button>
          </div>
        </section>
      )}

      {/* Click-outside backdrop for detail view */}
      {viewMode !== 'list' && (
        <div
        ...
        <div ref={contentRef} className="relative">
          ...
          {viewMode === 'list' && (
            <div className="space-y-6">
              ...
              {/* Paid Section — Table grouped by payment */}
              {paymentGroups.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <h2 style={{ color: colors.text }} className="text-sm font-semibold">
                      {t('deposit.sectionPaid')}
                    </h2>
                    <span
                      className="text-xs font-medium px-2 py-0.5 rounded-full"
          ...
          {viewMode === 'poDetail' && poDetailOrder && (() => {
            ...
            return (
              <DepositPODetailPanel
                ...
                depositDetails={matchedItem?.paymentDetails}
              />
            );
          })()}
        </div>
      ...
      <SecurityCodeDialog
        ...
        title={t('deposit.delete.title')}
        description={t('deposit.delete.description')}
