...
import { financeApi, type DepositListItem, type SubmitDepositPaymentRequest, type DepositPaymentItemRequest } from '@/lib/api/finance';
...
/** 5-decimal precision per MGMT rules */
...
export default function PaymentDialog({ open, onClose, selectedItems, onSuccess, t, theme }: PaymentDialogProps) {
  ...
  const { data: vendorBalance } = useQuery({
    queryKey: ['vendorBalance', vendorCode, paymentDate],
    queryFn: () => financeApi.getVendorBalance(vendorCode, paymentDate),
    enabled: open && !!vendorCode,
  });

  // --- Backend exchange rate fallback ---
  ...
  // --- Multi-source rate fetch (V1 faithful, 3-source cascade) ---
  ...
  // Auto-fetch when paymentDate rate selected
  useEffect(() => {
  ...
  const summary = useMemo(() => {
    ...
    selectedItems.forEach((item) => {
      ...
      const deposit = item.depositAmountUsd;
      const paid = item.actualPaidUsd;
  ...
  // --- Build request items (skip POs where cash=0 AND prepay=0 AND extra=0) ---
  const buildRequestItems = useCallback((): DepositPaymentItemRequest[] => {
    const extraPerOrder = selectedItems.length > 0 && showExtraFee && extraAmount > 0
      ? round5(extraAmount / selectedItems.length)
      : 0;
    ...
    return selectedItems
      .map((item) => {
        ...
        const deposit = item.depositAmountUsd;
        const paid = item.actualPaidUsd;
        const remaining = cfg.paymentMode === 'custom' ? cfg.customAmount : round5(deposit - paid);
        const prepay = cfg.usePrepay ? cfg.prepayAmount : 0;
        const cash = round5(Math.max(0, remaining - prepay));
        ...
        const reqItem: DepositPaymentItemRequest = {
          poNum: item.poNum,
      ...
      .filter((x): x is DepositPaymentItemRequest => x !== null);
  ...
  const submitMutation = useMutation({
    mutationFn: async (secCode: string) => {
      ...
      const request: SubmitDepositPaymentRequest = {
      ...
      return financeApi.submitDepositPayment(request, secCode);
    },
    onSuccess: () => {
      submitSecurity.onCancel();
      setSuccess(true);
  ...
  const submitSecurity = useSecurityAction({
    actionKey: 'btn_deposit_payment_submit',
    level: 'L2',
  ...
  if (success) {
    return (
      <ModalShell isOpen={open} onClose={onClose} title={t('deposit.payment.successTitle')} closable={false} showFooter={false}>
        <div className="text-center py-16">
          ...
          <h3 className="text-lg font-semibold mb-1" style={{ color: colors.text }}>{t('deposit.payment.successTitle')}</h3>
          <p className="text-sm" style={{ color: colors.textSecondary }}>{t('deposit.payment.successDescription')}</p>
        </div>
      </ModalShell>
    );
  }
  ...
  return (
    <>
        ...
        <div className="flex gap-6" style={{ minHeight: '400px' }}>
          ...
          <div className="w-[40%] shrink-0">
            <SectionHeader
              ...
              title={t('deposit.payment.selectedOrders')}
            />
                  ...
                  <div key={item.poNum} className="p-3 rounded-lg border" style={{ backgroundColor: `${colors.bgTertiary}80`, borderColor: colors.border }}>
                    ...
                    <div className="mt-2 pt-2 space-y-1.5" style={{ borderTop: `1px solid ${colors.border}` }}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {t('deposit.payment.modeOriginal')}
                        </button>
                        <button
                          type="button"
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mb-6">
              <SectionHeader
                ...
                title={t('deposit.payment.paymentDate')}
              />
              <input
                type="date"
                value={paymentDate}
            ...
            <div className="mb-6">
              <SectionHeader
                ...
                title={t('deposit.payment.exchangeRate')}
              />
            ...
            {vendorBalance && (
              <div className="mb-6">
                <SectionHeader
                  ...
                  title={t('deposit.payment.vendorBalance')}
                />
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-3">
                <SectionHeader
                  ...
                  title={t('deposit.payment.extraFee')}
                />
                <button
                  type="button"
              ...
              {showExtraFee && (
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-xs font-medium mb-1.5" style={{ color: colors.textSecondary }}>{t('deposit.payment.extraAmount')}</label>
                    <input
                      type="number"
                      step="0.01"
                      value={extraAmount || ''}
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1.5" style={{ color: colors.textSecondary }}>{t('deposit.payment.extraCurrency')}</label>
                    <select
                      value={extraCurrency}
                      onChange={(e) => setExtraCurrency(e.target.value as 'USD' | 'RMB')}
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1.5" style={{ color: colors.textSecondary }}>{t('deposit.payment.extraNote')}</label>
                    <input
                      type="text"
                      value={extraNote}
      ...
      <SecurityCodeDialog
        ...
        title={t('deposit.payment.securityTitle')}
        description={t('deposit.payment.securityDescription')}
