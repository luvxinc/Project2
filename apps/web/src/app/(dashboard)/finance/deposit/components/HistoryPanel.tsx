...
/**
 * HistoryPanel -- Two-column history view for deposit payments.
 * V1 parity: deposit/history.py deposit_payment_history_api
 *
 * Left:  Strategy versions (currency, deposit%, rate, mode changes)
 * Right: Payment versions (amount, rate, override, extra fee changes)
 */
export default function HistoryPanel({ pmtNo, poNum, t, theme }: HistoryPanelProps) {
  ...
  const { data: history, isLoading } = useQuery({
    queryKey: ['depositPaymentHistory', pmtNo, poNum],
    queryFn: () => financeApi.getDepositPaymentHistory(pmtNo, poNum),
  });

  const strategyVersions = history?.strategyVersions ?? [];
  const paymentVersions = history?.paymentVersions ?? [];
  ...
  if (strategyVersions.length === 0 && paymentVersions.length === 0) {
    return (
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div className="flex flex-col rounded-xl overflow-hidden" style={{ border: `1px solid ${colors.border}` }}>
        ...
        >
          ...
          <span className="text-xs font-semibold" style={{ color: colors.text }}>
            {t('deposit.history.strategyColumn')}
          </span>
          <span
        ...
        <div className="p-4 space-y-3 overflow-y-auto" style={{ maxHeight: '520px', backgroundColor: colors.bg }}>
          {strategyVersions.length === 0 ? (
            <p className="py-8 text-center text-sm" style={{ color: colors.textTertiary }}>
              {t('deposit.history.noStrategyVersions')}
            </p>
          ) : (
      ...
      <div className="flex flex-col rounded-xl overflow-hidden" style={{ border: `1px solid ${colors.border}` }}>
        ...
        >
          ...
          <span className="text-xs font-semibold" style={{ color: colors.text }}>
            {t('deposit.history.paymentColumn')}
          </span>
          <span
            className="ml-auto text-xs font-bold px-2 py-0.5 rounded-full"
        ...
        <div className="p-4 space-y-3 overflow-y-auto" style={{ maxHeight: '520px', backgroundColor: colors.bg }}>
          {paymentVersions.length === 0 ? (
            <p className="py-8 text-center text-sm" style={{ color: colors.textTertiary }}>
              {t('deposit.history.noPaymentVersions')}
            </p>
          ) : (
            paymentVersions.map((v, idx) => (
              <PaymentVersionCard key={idx} version={v} idx={idx} colors={colors} t={t} />
            ))
          )}
        </div>
      </div>
