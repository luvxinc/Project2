...
export function WarehouseCard({
  ...
  return (
    <div>
      {/* ═══ 3D Card (Hub-style tall rounded card) ═══ */}
      <div
        style={{
          backgroundColor: colors.bgSecondary,
          borderColor: colors.border,
        }}
        className="relative overflow-hidden rounded-[28px] border h-[420px] transition-transform duration-300 hover:scale-[1.02] group cursor-pointer"
        onClick={onEdit}
      >
        {/* 3D Scene filling the card */}
        ...
        {/* Stats overlay — bottom row with frosted glass */}
        <div
        ...
        >
          <div>
            ...
            <div className="text-[10px] text-white/60">{t('shelf.card.locations')}</div>
          </div>
          <div className="w-px h-6 bg-white/20" />
          <div>
            <div className="text-[18px] font-bold text-white">{aisleCount}</div>
            <div className="text-[10px] text-white/60">{t('shelf.card.aisles')}</div>
          </div>
          <div className="w-px h-6 bg-white/20" />
          <div>
            <div className="text-[18px] font-bold text-white">{bayCount}</div>
            <div className="text-[10px] text-white/60">{t('shelf.card.bays')}</div>
          </div>
          <div className="w-px h-6 bg-white/20" />
          <div>
            ...
            <div className="text-[10px] text-white/60">{t('shelf.card.levels')}</div>
          </div>
        </div>
      </div>

      {/* ═══ Info Below Card (Hub-style) ═══ */}
      <div className="mt-4 text-center">
        ...
        <p style={{ color: colors.textSecondary }} className="text-[13px] mb-3">
        ...
        <div className="flex items-center justify-center gap-2">
          <button
            onClick={onEdit}
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-[13px] font-medium transition-all hover:opacity-90"
          ...
          <button
            onClick={onDownload}
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-[13px] font-medium transition-all hover:opacity-80"
            style={{ backgroundColor: colors.bgTertiary, color: colors.textSecondary }}
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            {t('shelf.card.download')}
          </button>
          ...
          <button
            onClick={onDelete}
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-[13px] font-medium transition-all hover:opacity-80"
