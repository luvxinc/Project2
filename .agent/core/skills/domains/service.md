...
|--------|--------|---------|---------|
| `Kotlin`, `Spring Boot`, `模块` | 后端架构师 | `core/skills/backend.md` §3 | DDD 分层/Controller/Service/Repository |
| `构建`, `Gradle`, `依赖` | 后端架构师 | `core/skills/backend.md` §2 | 多模块构建/依赖管理 |
| `事务`, `Transaction`, `UseCase` | 后端架构师 | `core/skills/backend.md` §5 | 事务边界/回滚/嵌套事务 |
| `JUnit`, `测试`, `Testcontainers` | 后端架构师 | `core/skills/backend.md` §6 | 单元/集成/E2E 测试规范 |
| `配置`, `profiles`, `环境` | 后端架构师 | `core/skills/backend.md` §7 | 多环境配置/密钥注入 |
