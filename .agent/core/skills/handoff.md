---
name: handoff
description: 会话交接协议 — 长任务跨会话接力, 检查点保存/恢复, 上下文压缩传递。
---

# 会话交接协议 (Session Handoff Protocol)

> **问题: 上下文窗口有限, 长任务无法在一个会话完成。**
> **方案: 标准化的检查点机制, 让新会话能精确恢复上一次的进度。**

---

## 1. 何时触发交接

| 信号 | 行动 |
|------|------|
| 上下文使用率 > 70% | 🟠 准备检查点 |
| 上下文使用率 > 85% | 🔴 立即保存, 结束会话 |
| 任务预计步骤 > 10 | 🟡 提前规划切分点 |
| Agent 开始重复/遗忘 | 🔴 立即保存, 结束会话 |
| 用户说 "先到这里" | 保存检查点 |

---

## 2. 检查点格式

```markdown
# 🔖 Checkpoint: {任务名称}

> 保存时间: YYYY-MM-DD HH:MM
> 会话 ID: {conversation-id}
> Spec 文件: projects/{project}/data/specs/{spec-file}.md

## 已完成 ✅
1. ✅ Step 1: {描述} — {关键输出}
2. ✅ Step 2: {描述} — {关键输出}

## 进行中 🔄
3. 🔄 Step 3: {描述}
   - 已做: {做了什么}
   - 未做: {还差什么}
   - 关键文件: `path/to/file.ts` (第 XX 行)

## 待做 ⬜
4. ⬜ Step 4: {描述}
5. ⬜ Step 5: {描述}

## 关键上下文 (新会话必读)
- 当前在 Phase {X} — 参考 `projects/{project}/roadmap.md`
- 正在使用实施方案: `playbooks/{name}.md`
- 数据模型变更: {是/否, 如是说明}
- 已知陷阱: {遇到的问题 + 解法}

## 相关文件 (新会话应先读)
- `path/to/modified-file-1.ts`
- `path/to/modified-file-2.prisma`
- `projects/{project}/data/specs/{spec-file}.md`
```

---

## 3. 存储位置

```
projects/{project}/data/checkpoints/YYYY-MM-DD_{task-name}_checkpoint.md
```

> **命名和生命周期规则: `project-structure.md` §3.6 + §4**

---

## 4. 恢复流程

新会话开始时, 如果用户说 "继续上次" 或引用之前的任务:

```
1. 读 projects/{project}/data/checkpoints/ 最新的检查点文件
2. 读检查点中列出的相关文件
3. 读对应的 Spec 文件 (projects/{project}/data/specs/)
4. 向用户确认: "上次做到 Step 3, 我从这里继续?"
5. 恢复执行
```

### 恢复铁律

| 规则 | 说明 |
|------|------|
| **不要假设** | 读完检查点后, 先确认再继续 |
| **验证现状** | 代码可能被用户手动改过, 先检查 git status |
| **重读 Spec** | Spec 是真相源, 不依赖上下文记忆 |

---

## 5. 主动切分 (长任务规划)

对于预估 > 10 步的任务, 在 Wizard Phase 2 (Spec) 时主动规划切分点:

```markdown
## 4. 执行步骤 (含切分点)

### Session 1: 数据层
- [ ] Step 1: Schema 修改
- [ ] Step 2: Migration 创建
- [ ] Step 3: Repository 层
📌 切分点 A — 保存检查点

### Session 2: 业务层
- [ ] Step 4: Service 层
- [ ] Step 5: Controller 层
- [ ] Step 6: DTO + 验证
📌 切分点 B — 保存检查点

### Session 3: 前端
- [ ] Step 7: 页面 UI
- [ ] Step 8: API 调用
- [ ] Step 9: i18n
- [ ] Step 10: 验证
```

---

## 6. 紧急交接 (上下文爆满)

如果上下文突然接近极限, 用最短格式:

```markdown
# 🚨 Emergency Checkpoint
任务: {一句话}
做到: Step {N}
下一步: {一句话}
关键文件: `path/to/file`
Spec: `projects/{project}/data/specs/{file}.md`
```

---

## 7. 与全局工作流的关系

> **检查点保存不改变任务状态。恢复后从当前状态继续流转。**
> **状态机定义: `workflows/build.md` §0**

---

## 8. L3 工具库引用

| 场景 | 推荐加载 | 文件路径 | 作用 |
|------|---------|---------|------|
| 上下文压缩技巧 | ECC: Hooks | `warehouse/tools/everything-claude-code/02-rules-hooks.md` §2 | 预提交/后保存钩子模式 |

---

*Version: 1.1.0 — Added 工作流引用 + L3 工具*
*Updated: 2026-02-12*
