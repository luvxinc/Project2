...
一键 QA Gate：`bash .agent/core/scripts/qa-gate.sh .`

不快乐路径：每次关键功能审计至少覆盖 3 个异常场景（慢网/超时、500/401、重复提交）。

反死循环检查：确认遵守 `core/rules/common.md` §10。

安全增强审计：`bash .agent/core/scripts/security-extra-audit.sh`（SAST/Secret scan/SBOM/License）。
...
归档 → `.agent/projects/{project}/data/errors/{CODE}-{NNN}.md`
...
培训记录 → `.agent/projects/{project}/data/training/{YYYY-MM-DD}_{topic}.md`
...
| 知识类型 | 来源 | 存储位置 |
|----------|------|----------|
| 常见错误 | 审计发现 | `.agent/projects/{project}/data/errors/` |
...
*Version: 4.0.0 — Phase 1 瘦身 (ASCII图删除 + 影响半径外引 + 性能测试/混沌工程外迁reference/)*
*Updated: 2026-02-19*
