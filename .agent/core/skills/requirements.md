...
```
1. 读 `projects/{project}/CONTEXT.md` §2 “当前阶段指针”
2. 确认当前运行栈:
   - V2: NestJS + Prisma + PostgreSQL → 写 TypeScript, 用 Prisma Schema
   - V3: Spring Boot + Kotlin + JPA → 写 Kotlin, 用 Flyway Migration
3. 后续所有代码按此栈写
4. ⚠️ V2 代码禁止用 V3 语法 (Kotlin/Spring), 反之亦然
```
...
| **相关实施方案** | 检查 `projects/{project}/playbooks/` | 是否有项目级实施方案 |
...
> **命名和生命周期规则: `project-structure.md` §3.3 + §4**
