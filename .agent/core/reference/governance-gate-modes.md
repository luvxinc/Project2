...
1. init contract
```bash
core/scripts/scope-contract-init.sh .agent/projects/{project} {task-id} {base-ref} '^\.agent/core/scripts/.*' '^\.agent/core/reference/.*'
```
2. gate run with contract
```bash
QA_SCOPE_CONTRACT_FILE=.agent/projects/{project}/data/tmp/{task-id}/scope-contract.txt core/scripts/qa-gate.sh
```
