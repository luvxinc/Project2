...
| 不安全依赖 | `npm audit` / `./gradlew dependencyCheck` 高危即修 |
| CORS | 必须配置白名单, 禁止 `*` |
| 限流 | 公开端点必须有 throttle |
...
| 阶段 | 命令 | 通过标准 | 失败 |
|------|------|---------|------|
| 1. 编译 | `./gradlew build` / `pnpm build` | BUILD SUCCESSFUL / ✓ Compiled | **STOP** |
| 2. 类型 | `pnpm tsc --noEmit` | 零错误 | 修复 |
| 3. Lint | `pnpm lint` | 零错误 | 修复 |
| 4. 测试 | `pnpm test` / `./gradlew test` | 全通过 | 修复 |
| 5. 覆盖率 | `--coverage` | ≥80% | 补测试 |
| 6. 安全 | `npm audit` | 无高危 | 修复 |

**关键: 阶段 1 失败 = 后续全部 STOP, 不继续。**
**🔴 铁律: 集成测试 (阶段 4) 不通过 = 禁止提交完工报告, 发回工程师重做。(详见 `.agent/projects/mgmt/reference/iron-laws.md` §4)**
...
*Version: 2.0.0 — 强化版 (集成测试门禁 + 代码拆分+复用 + 共享模块追溯)*
...
```
新建文件前的强制检查:
  1. grep_search 搜索相似功能 → 是否已有现成实现?
  2. 检查 shared/utils, common/ 下是否有可复用的工具函数
  3. 检查同模块其他文件是否有可提取的共享逻辑
  
  如果找到可复用代码:
    → 导入使用, 不重新实现
    → 如果需要微调: 在原有基础上扩展, 不复制粘贴
  
  如果确认没有:
    → 新建文件, 但考虑: 这个逻辑将来会被其他地方用吗?
    → 如果是 → 放入 shared/ 或 common/, 不放在模块私有目录
```

### 9.3 复用追溯 (修改共享代码时必做)

```
修改 shared/, common/, utils/ 等共享模块时:
  1. 🔴 grep 搜索所有消费此模块的文件
  2. 列出所有消费者 (不能遗漏)
  3. 逐一验证:
     → 接口签名变了? 消费者要更新
     → 行为变了? 消费者的调用逻辑要验证
     → 未涉及的消费者也要抽查 — 隐式依赖可能破坏
  4. 全部验证通过后才提交

  ⚠️ 特别注意: 工具函数的返回值类型变更、参数顺序变更
  → 编译器可能不报错 (any/unknown 类型), 但运行时会崩
```

> **此规则与 §6 跨文件影响分析配合使用。§6 查"改了谁", §9 查"能复用什么"。**
