...
| 🔴 **功能性验证门禁** | **编译通过 ≠ 功能正常。** 每次修改完成后, 必须执行功能性验证: ① **前端 UI:** 必须检查 DOM 结构 (overflow 裁剪, z-index 层叠, 绝对定位溢出) + 数据是否正确填充到 UI 组件 ② **后端 API:** 发送实际 HTTP 请求验证响应结构和数据 ③ **集成:** 验证前后端数据流 E2E 可达。**编译通过但功能不可用 = Block, 不得交付** |
| 🔴 **交付前用户需求对照** | **提交交付报告前, 必须逐条对照用户原始需求, 用自检清单确认:** 每个需求点是否已实现 + 是否已功能验证。任何未验证项 = 不得声称交付 |
...
```
📋 交付闸门 — 必须全部 ✅ 才可交付:
├── [ ] 编译通过 (tsc / gradlew compileKotlin)
├── [ ] 集成测试通过 (如有)
├── [ ] 🔴 功能性验证: 在 running app 上确认功能可用
│       前端: UI 渲染正确, 数据展示正确, 交互正常
│       后端: API 返回预期数据
├── [ ] 🔴 用户需求逐条对照: 每个需求点已实现+已验证
├── [ ] 无 overflow/z-index/裁剪 等 CSS 布局问题 (前端)
└── [ ] 变更摘要准确反映实际改动
```
