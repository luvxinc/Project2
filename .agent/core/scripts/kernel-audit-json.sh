...
PROJECT_ROOT="${2:-$REPO_ROOT/.agent/projects/mgmt}"
TMP_DIR="${TMPDIR:-/tmp}"
...
j1=$(run_step agentDocAudit "$REPO_ROOT/.agent/core/scripts/agent-doc-audit.sh" "$REPO_ROOT/.agent")
j2=$(run_step memoryDedupeAudit "$REPO_ROOT/.agent/core/scripts/memory-dedupe-audit.sh" "$PROJECT_ROOT")
j3=$(run_step artifactLifecycleAudit "$REPO_ROOT/.agent/core/scripts/artifact-lifecycle-audit.sh" "$PROJECT_ROOT" --tmp-only)
