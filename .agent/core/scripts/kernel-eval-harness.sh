...
SCRIPTS="$REPO_ROOT/.agent/core/scripts"
...
run_case memory_dedupe "$SCRIPTS/memory-dedupe-audit.sh" "$REPO_ROOT/.agent/projects/mgmt"
...
out=subprocess.check_output(["$SCRIPTS/kernel-audit-json.sh","$REPO_ROOT","$REPO_ROOT/.agent/projects/mgmt"],text=True)
obj=json.loads(out)
