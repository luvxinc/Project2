...
## 🔴 第五法则: 越南语 Fallback 铁律

> **越南语 (VI) 只在 VMA 模块维护真实翻译。其他所有模块 VI 自动 fallback 到 EN。**

| 约定 | 值 |
|------|-----|
| VMA 模块 (`vma.json`) | ✅ 维护真实越南语翻译 |
| 其他模块 (`products`, `users`, `common`, ...) | ❌ 不维护 VI — 自动使用 EN |
| 实现位置 | `apps/web/src/i18n.ts` — VI 命名空间映射 |
| 新增模块 | 只需维护 EN + ZH, VI 自动走 EN |

### 规则

1. **新建模块** — 只需创建 `en/{module}.json` + `zh/{module}.json`, **不需要** `vi/{module}.json`
2. **修改 i18n.ts** — VI 命名空间中, 非 VMA 一律引用 `en` 对应 import
3. **VMA 模块** — 是唯一例外, 必须维护 `vi/vma.json` 的真实越南语

### 血泪教训

| 日期 | 事故 | 原因 | 后果 |
|------|------|------|------|
| 2026-02-17 | 产品模块越南语页面显示残缺 | VI json 缺少大量 key | 用户看到空白/key 名 |

> **交叉引用:** `conventions.md` §2 i18n 规范
