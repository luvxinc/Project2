...
> **Purpose**: 此文件定义了 MGMT ERP 的统一审计日志系统（Unified Logging System V6.0）。它规范了如何通过 `app.log`, `audit.log`, `error.log` 记录业务操作、底层审计和系统异常。
> **AI Attention**: 所有新增的业务逻辑（CRUD, Sensitive Actions）**必须** 接入 `audit_logger`。不允许使用 `print()` 或原生 `logging`。必须正确设置 `AuditContextManager`。
> **Constraints**: `HARD` - 所有业务操作必须审计。
> **Related Files**: 
> - `aid/system/数据库表.md`: `System_Audit_Log_Django` 用途。
