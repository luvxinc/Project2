...
> **Purpose**: 此文件指导 AI Agent 和开发者如何正确连接 MGMT ERP 数据库。它定义了 `DBClient` 的标准用法、独立脚本的编写规范以及 `django-extensions` 的使用。
> **AI Attention**: 任何涉及数据库读写的代码生成（App内或独立脚本），**必须** 遵循此文档中的 `DBClient` 模式。严禁使用原生 `pymysql` 或未经封装的连接方式。
> **Constraints**: `HARD` - 必须使用 provided connection patterns.
> **Related Files**: 
> - `aid/system/数据库表.md`: 查询表结构定义。
> - `aid/mistake/Collate问题报错.md`: 避免 Collation 错误。
